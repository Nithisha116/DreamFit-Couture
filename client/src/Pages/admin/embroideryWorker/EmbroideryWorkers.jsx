// // pages/embroidery-workers/EmbroideryWorkers.jsx
...
//   X
// } from "lucide-react";
// import { fetchAllEmbroideryWorkers, deleteEmbroideryWorker, fetchEmbroideryWorkerStats } from "../../../features/embroideryWorker/embroideryWorkerSlice";
// import showToast from "../../../utils/toast";
// import StatusBadge from "../../../components/common/StatusBadge";
// import LeaveStatusModal from "../../../components/modals/LeaveStatusModal";

// export default function EmbroideryWorkers() {
//   const navigate = useNavigate();
//   const dispatch = useDispatch();
  
//   const { embroideryWorkers, embroideryWorkerStats, loading, pagination } = useSelector((state) => state.embroideryWorker);
...
//   const [selectedEmbroideryWorker, setSelectedEmbroideryWorker] = useState(null);
...
//   // Fetch embroideryWorkers with pagination
//   useEffect(() => {
//     dispatch(fetchAllEmbroideryWorkers({ 
//       page: currentPage,
//       limit: itemsPerPage,
//       search: searchTerm, 
//       status: statusFilter,
...
//     }));
//     dispatch(fetchEmbroideryWorkerStats());
//   }, [dispatch, currentPage, itemsPerPage, searchTerm, statusFilter, availabilityFilter]);

//   // Handle Add Embroidery Worker - with basePath
//   const handleAddEmbroideryWorker = () => {
//     navigate(`${basePath}/embroidery-workers/add`);
//   };

//   // Handle View EmbroideryWorker - with basePath
//   const handleViewEmbroideryWorker = (id) => {
//     navigate(`${basePath}/embroidery-workers/${id}`);
//   };

//   // Handle Edit Embroidery Worker - with basePath
//   const handleEditEmbroideryWorker = (id) => {
//     if (canEdit) {
//       navigate(`${basePath}/embroidery-workers/edit/${id}`);
//     } else {
//       showToast.error("You don't have permission to Edit Embroidery Workers");
//     }
//   };

//   const handleDeleteClick = (embroideryWorker) => {
//     setSelectedEmbroideryWorker(embroideryWorker);
//     setShowDeleteModal(true);
//   };

//   const handleDeleteConfirm = async () => {
//     try {
//       await dispatch(deleteEmbroideryWorker(selectedEmbroideryWorker._id)).unwrap();
//       showToast.success("Embroidery Worker deleted successfully");
//       setShowDeleteModal(false);
//       setSelectedEmbroideryWorker(null);
//     } catch (error) {
//       showToast.error(error || "Failed to Delete Embroidery Worker");
//     }
//   };

//   const handleLeaveUpdate = (embroideryWorker) => {
//     setSelectedEmbroideryWorker(embroideryWorker);
...
//           <h1 className="text-lg font-black text-slate-800">Embroidery Workers</h1>
//           <div className="flex items-center gap-2">
//             {canAdd && (
//               <button
//                 onClick={handleAddEmbroideryWorker}
...
//             <input
//               type="text"
//               placeholder="Search embroideryWorkers..."
//               value={searchTerm}
...
//             <div className="text-xs font-bold text-blue-600">{embroideryWorkerStats?.total || 0}</div>
//             <div className="text-[10px] text-slate-500 truncate">Total</div>
//           </div>
//           <div className="text-center">
//             <div className="text-xs font-bold text-green-600">{embroideryWorkerStats?.present || 0}</div>
//             <div className="text-[10px] text-slate-500 truncate">Present</div>
//           </div>
//           <div className="text-center">
//             <div className="text-xs font-bold text-red-600">{embroideryWorkerStats?.onLeave || 0}</div>
//             <div className="text-[10px] text-slate-500 truncate">Leave</div>
//           </div>
//           <div className="text-center">
//             <div className="text-xs font-bold text-orange-600">{embroideryWorkerStats?.halfDay || 0}</div>
//             <div className="text-[10px] text-slate-500 truncate">Half Day</div>
//           </div>
//           <div className="text-center">
//             <div className="text-xs font-bold text-purple-600">{embroideryWorkerStats?.available || 0}</div>
...
//               <div>
//                 <p className="text-sm text-slate-500">Total Embroidery Workers</p>
//                 <p className="text-3xl font-black text-slate-800">{embroideryWorkerStats?.total || 0}</p>
//               </div>
...
//                 <p className="text-3xl font-black text-green-700">{embroideryWorkerStats?.present || 0}</p>
//               </div>
...
//                 <p className="text-3xl font-black text-red-700">{embroideryWorkerStats?.onLeave || 0}</p>
//               </div>
...
//                 <p className="text-3xl font-black text-orange-700">{embroideryWorkerStats?.halfDay || 0}</p>
//               </div>
...
//                 <p className="text-3xl font-black text-purple-700">{embroideryWorkerStats?.available || 0}</p>
//               </div>
...
//               Embroidery Workers Management
//             </h1>
//             <p className="text-slate-500 font-medium">Manage all Embroidery Workers and their work assignments</p>
//           </div>
//           {canAdd && (
//             <button
//               onClick={handleAddEmbroideryWorker}
...
//             {embroideryWorkers?.length || 0} embroideryWorkers
//           </span>
//         </div>
...
//             <p className="mt-2 text-sm text-slate-500">Loading Embroidery Workers...</p>
//           </div>
//         ) : embroideryWorkers?.length > 0 ? (
//           <>
//             {/* ===== MOBILE GRID VIEW - COMPLETELY CENTERED ===== */}
//             {mobileView === "grid" && (
//               <div className="lg:hidden grid grid-cols-1 sm:grid-cols-2 gap-3">
//                 {embroideryWorkers.map((embroideryWorker) => (
//                   <div
//                     key={embroideryWorker._id}
...
//                           {embroideryWorker.name?.charAt(0)}
//                         </div>
//                         <div className="min-w-0">
//                           <p className="font-medium text-slate-800 text-sm truncate max-w-[100px]">{embroideryWorker.name}</p>
//                           <p className="text-[10px] text-slate-400 truncate">{embroideryWorker.embroideryWorkerId}</p>
//                         </div>
//                       </div>
//                       <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${getLeaveStatusColor(embroideryWorker.leaveStatus)}`}>
//                         {embroideryWorker.leaveStatus}
//                       </span>
//                     </div>

//                     {/* Contact and Specialization */}
//                     <div className="space-y-1 mb-2">
//                       <p className="text-xs text-slate-600 truncate">{embroideryWorker.phone}</p>
//                       <div className="flex flex-wrap gap-1">
//                         {embroideryWorker.specialization?.slice(0, 2).map((spec, idx) => (
//                           <span key={idx} className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded-full text-[8px]">
//                             {spec}
//                           </span>
//                         ))}
//                         {embroideryWorker.specialization?.length > 2 && (
//                           <span className="text-[8px] text-slate-400">+{embroideryWorker.specialization.length - 2}</span>
//                         )}
//                       </div>
//                     </div>
...
//                       <div>
//                         <div className="font-bold text-blue-600">{embroideryWorker.workStats?.totalAssigned || 0}</div>
//                         <div className="text-slate-500">Total</div>
//                       </div>
//                       <div>
//                         <div className="font-bold text-green-600">{embroideryWorker.workStats?.completed || 0}</div>
//                         <div className="text-slate-500">Done</div>
//                       </div>
//                       <div>
//                         <div className="font-bold text-orange-600">{embroideryWorker.workStats?.inProgress || 0}</div>
...
//                       <button
//                         onClick={() => handleViewEmbroideryWorker(embroideryWorker._id)}
...
//                       {canEdit && (
//                         <button
//                           onClick={() => handleLeaveUpdate(embroideryWorker)}
...
//                       {canEdit && (
//                         <button
//                           onClick={() => handleEditEmbroideryWorker(embroideryWorker._id)}
...
//                       {canDelete && (
//                         <button
//                           onClick={() => handleDeleteClick(embroideryWorker)}
...
//                         </div>
//                         <div className="min-w-0">
//                           <p className="font-medium text-slate-800 text-sm truncate">{embroideryWorker.name}</p>
//                           <p className="text-xs text-slate-500 truncate">{embroideryWorker.phone}</p>
//                         </div>
//                       </div>
//                       <div className="flex items-center gap-1 flex-shrink-0">
//                         <span className={`px-2 py-1 rounded-full text-[10px] font-medium whitespace-nowrap ${getLeaveStatusColor(embroideryWorker.leaveStatus)}`}>
//                           {embroideryWorker.leaveStatus}
//                         </span>
//                       </div>
//                     </div>
...
//                       <div className="text-slate-500 truncate max-w-[100px]">ID: {embroideryWorker.embroideryWorkerId}</div>
//                       <div className="flex items-center gap-2">
//                         <span className="text-blue-600 font-medium">{embroideryWorker.workStats?.totalAssigned || 0} works</span>
//                         <button
//                           onClick={() => handleViewEmbroideryWorker(embroideryWorker._id)}
...
//                     {embroideryWorkers.map((embroideryWorker) => (
//                       <tr key={embroideryWorker._id} className="hover:bg-slate-50 transition-all">
//                         <td className="px-6 py-4 font-mono font-bold text-blue-600">
//                           {embroideryWorker.embroideryWorkerId}
//                         </td>
...
//                             </div>
//                             <div className="min-w-0">
//                               <p className="font-medium text-slate-800 truncate max-w-[150px]">{embroideryWorker.name}</p>
//                               <p className="text-xs text-slate-400 truncate">{embroideryWorker.email || 'No email'}</p>
//                             </div>
//                           </div>
//                         </td>
//                         <td className="px-6 py-4 text-slate-600">
//                           {embroideryWorker.phone}
//                         </td>
//                         <td className="px-6 py-4">
//                           <div className="flex flex-wrap gap-1">
//                             {embroideryWorker.specialization?.slice(0, 2).map((spec, idx) => (
...
//                             ))}
//                             {embroideryWorker.specialization?.length > 2 && (
//                               <span className="text-xs text-slate-400">+{embroideryWorker.specialization.length - 2}</span>
//                             )}
//                           </div>
//                         </td>
//                         <td className="px-6 py-4">
//                           <span className="font-medium">{embroideryWorker.experience || 0} years</span>
//                         </td>
//                         <td className="px-6 py-4">
//                           <div className="flex items-center gap-2">
//                             {getLeaveStatusIcon(embroideryWorker.leaveStatus)}
//                             <span className={`text-sm font-medium ${
//                               embroideryWorker.leaveStatus === 'present' ? 'text-green-600' :
//                               embroideryWorker.leaveStatus === 'leave' ? 'text-red-600' :
//                               embroideryWorker.leaveStatus === 'half-day' ? 'text-orange-600' :
//                               'text-purple-600'
//                             }`}>
//                               {embroideryWorker.leaveStatus?.replace('-', ' ')}
//                             </span>
//                           </div>
//                           {!embroideryWorker.isAvailable && (
...
//                               <span className="font-bold">{embroideryWorker.workStats?.totalAssigned || 0}</span>
//                             </div>
//                             <div className="flex items-center gap-2 text-xs">
//                               <span className="text-green-600">✓ Completed:</span>
//                               <span className="font-bold">{embroideryWorker.workStats?.completed || 0}</span>
//                             </div>
//                             <div className="flex items-center gap-2 text-xs">
//                               <span className="text-orange-600">⚡ In Progress:</span>
//                               <span className="font-bold">{embroideryWorker.workStats?.inProgress || 0}</span>
//                             </div>
//                             <div className="flex items-center gap-2 text-xs">
//                               <span className="text-yellow-600">⏳ Pending:</span>
//                               <span className="font-bold">{embroideryWorker.workStats?.pending || 0}</span>
//                             </div>
//                           </div>
//                         </td>
//                         <td className="px-6 py-4">
...
//                             {(canEdit || embroideryWorker._id === user?.embroideryWorkerId) && (
//                               <button
//                                 onClick={() => handleLeaveUpdate(embroideryWorker)}
...
//           // Empty State
//           <div className="bg-white rounded-xl p-8 text-center">
//             <Palette size={40} className="text-slate-300 mx-auto mb-4" />
//             <p className="text-slate-500 text-base">No Embroidery Workers found</p>
//             {canAdd && (
//               <button
//                 onClick={handleAddEmbroideryWorker}
...
//               >
//                 <Plus size={16} />
//                 Add Your First Embroidery Worker
//               </button>
//             )}
//           </div>
//         )}
...
//       {showDeleteModal && selectedEmbroideryWorker && (
...
//               </div>
//               <h2 className="text-xl lg:text-2xl font-black text-center text-slate-800 mb-2">Delete Embroidery Worker</h2>
//               <p className="text-sm lg:text-base text-center text-slate-500 mb-4 lg:mb-6">
//                 Are you sure you want to delete <span className="font-black text-slate-700">{selectedEmbroideryWorker.name}</span>?
...
//       {/* Leave Status Modal */}
//       {showLeaveModal && selectedEmbroideryWorker && (
//         <LeaveStatusModal
//           embroideryWorker={selectedEmbroideryWorker}
//           onClose={() => {
...
export default function EmbroideryWorkers() {
  ...
  // 🚀 OPTIMIZED: Fetch embroideryWorkers with pagination - only when debounced search changes
  useEffect(() => {
    dispatch(fetchAllEmbroideryWorkers({ 
      page: currentPage,
      limit: itemsPerPage,
  ...
  // Handle Add Embroidery Worker - with basePath
  const handleAddEmbroideryWorker = useCallback(() => {
    navigate(`${basePath}/embroidery-workers/add`);
  }, [navigate, basePath]);

  // Handle View EmbroideryWorker - with basePath
  const handleViewEmbroideryWorker = useCallback((id) => {
    navigate(`${basePath}/embroidery-workers/${id}`);
  }, [navigate, basePath]);

  // Handle Edit Embroidery Worker - with basePath
  const handleEditEmbroideryWorker = useCallback((id) => {
    if (canEdit) {
      navigate(`${basePath}/embroidery-workers/edit/${id}`);
    } else {
      showToast.error("You don't have permission to Edit Embroidery Workers");
  ...
  const handleDeleteClick = useCallback((embroideryWorker) => {
    setSelectedEmbroideryWorker(embroideryWorker);
    setShowDeleteModal(true);
  }, []);
  ...
  }, [dispatch, selectedEmbroideryWorker]);

  const handleLeaveUpdate = useCallback((embroideryWorker) => {
    setSelectedEmbroideryWorker(embroideryWorker);
    setShowLeaveModal(true);
  }, []);
  ...
  const isLoading = loading && embroideryWorkers?.length === 0;
            ...
            <div className="hidden lg:block bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="px-6 py-4 text-left text-xs font-black text-slate-500 uppercase tracking-wider">Embroidery Worker ID</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {embroideryWorkers.map((embroideryWorker) => (
                      <tr key={embroideryWorker._id} className="hover:bg-slate-50 transition-all">
                        <td className="px-6 py-4 font-mono font-bold text-blue-600">{embroideryWorker.embroideryWorkerId}</td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center text-white font-bold flex-shrink-0">
                              {embroideryWorker.name?.charAt(0)}
                        ...
                        <td className="px-6 py-4 text-slate-600">{embroideryWorker.phone}</td>
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1">
                            {embroideryWorker.specialization?.slice(0, 2).map((spec, idx) => (
                              <span key={idx} className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-medium whitespace-nowrap">
